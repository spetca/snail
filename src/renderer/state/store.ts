...
export const useStore = create<AppState>((set, get) => ({
  ...
  setPowerMin: (powerMin) => { if (Number.isFinite(powerMin)) set(s => ({ powerMin: Math.min(powerMin, s.powerMax - 1) })) },
  setPowerMax: (powerMax) => { if (Number.isFinite(powerMax)) set(s => ({ powerMax: Math.max(powerMax, s.powerMin + 1) })) },
  setScrollOffset: (scrollOffset) => set((s) => withViewport(s, { scrollOffset })),
  setXAxisMode: (xAxisMode) => set({ xAxisMode }),
  setSampleRate: (sampleRate) => { if (Number.isFinite(sampleRate) && sampleRate > 0) set((s) => withViewport(s, { sampleRate })) },
  ...
  setTu: (tu) => { if (Number.isSafeInteger(tu) && tu > 0) set({ tu, correlationData: null }) },
  setCpLen: (cpLen) => { if (Number.isSafeInteger(cpLen) && cpLen > 0) set({ cpLen, correlationData: null }) },
